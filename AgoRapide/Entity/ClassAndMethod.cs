...
namespace AgoRapide {
    [AgoRapide(
        ...
        LongDescription =
            ...
            "(but this should be the exception, usually -" + nameof(Request<CoreProperty>.CurrentUser) + "-'s -" + nameof(DBField.id) + "- " +
    ...
    public class ClassAndMethod<TProperty> : ApplicationPart<TProperty> where TProperty : struct, IFormattable, IConvertible, IComparable { // What we really would want is "where T : Enum"
    }
}
