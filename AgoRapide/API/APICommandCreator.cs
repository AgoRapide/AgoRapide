...
namespace AgoRapide.API {
    ...
    [Class(
        Description =
            "This convenience class serves the following purposes:\r\n" +
            ...
            "\r\n" +
            "API URLs and API links are created through an instance because -" + nameof(ResponseFormat) + "- is needed \r\n" +
            ...
            "\r\n" +
            "Note how only static instances are offered, like -" + nameof(HTMLInstance) + "- and -" + nameof(JSONInstance) + "- (since constructor is private)."
    )]
    public class APICommandCreator {
        ...
        /// <summary>
        /// Creates HTML API link for <paramref name="apiCommand"/> by prepending <see cref="ConfigurationAttribute.BaseUrl"/> 
        /// (through Util.Configuration.C.BaseUrl) and appending <see cref="ResponseFormat"/>
        /// 
        /// Example: With <paramref name="apiCommand"/> "Person/42" will return something like "https://sample.AgoRapide.com/api/Person/42/HTML".
        /// </summary>
        /// <param name="apiCommand">Note that if already starts with http:// or https:// then <see cref="ConfigurationAttribute.BaseUrl"/> will not be prepended.</param>
        /// <returns></returns>
        public Uri CreateAPIUrl(string apiCommand) => CreateAPIUrl(apiCommand, ResponseFormat);

        /// <summary>
        /// Creates HTML API link for <paramref name="apiCommand"/> by prepending <see cref="ConfigurationAttribute.BaseUrl"/> 
        /// (through Util.Configuration.C.BaseUrl) and appending <paramref name="responseFormat"/>
        /// 
        /// Example: With <paramref name="apiCommand"/> "Person/42" will return something like "https://sample.AgoRapide.com/api/Person/42/HTML".
        /// </summary>
        /// <param name="apiCommand">Note that if already starts with http:// or https:// then <see cref="ConfigurationAttribute.BaseUrl"/> will not be prepended.</param>
        ...
        /// (through Util.Configuration.C.BaseUrl) and appending <see cref="ResponseFormat"/>
        /// 
        /// Example: With <paramref name="apiCommand"/> "Person/42" will return something like {a href="https://sample.AgoRapide.com/api/Person/42/HTML"}Person/42{/a}
        /// </summary>
        /// <param name="apiCommand">Note that if already starts with http:// or https:// then <see cref="ConfigurationAttribute.BaseUrl"/> will not be prepended.</param>
        /// <param name="linkText">May be null in which case <paramref name="apiCommand"/> will be used.</param>
        /// <returns></returns>
