...
namespace AgoRapide.API {
    public abstract class BaseController : ApiController {
        ...
        public object AgoRapideGenericMethod(APIMethod method, BaseEntity currentUser) {
            ...
            var tip = request.ResponseFormat == ResponseFormat.HTML ? "\r\nTip: Add " + Util.Configuration.A.HTMLPostfixIndicator + " at the end of your request URL if you want this response in HTML-format instead of JSON" : "";
            ...
            if (!string.IsNullOrEmpty(Util.Configuration.A.ApiPrefix) && Util.Configuration.A.ApiPrefix.Length > 1) { // In principle length is guaranteed to be more than one when not empty
                ...
                if (!url.ToLower().Contains(prefix)) {
                    var suggestedUrl = Util.Configuration.A.RootUrl + Util.Configuration.A.ApiPrefix;
                    request.Result.ResultCode = ResultCode.client_error;
                    request.Result.AddProperty(CoreP.Message.A(), "Did you remember '" + Util.Configuration.A.ApiPrefix + "' in your URL?" + tip);
            ...
            if (exactMatch != null) {
                ...
                if (exactMatch.Value.method.Origin != APIMethodOrigin.Autogenerated) throw new Request.MethodMatchingException("Found " + nameof(exactMatch) + " for " + e.method.Name + " with " + nameof(e.method.Origin) + " " + e.method.Origin + ". These methods are supposed to be handled by the 'ordinary' API .NET routing mechanism.");
                switch (e.method.A.CoreMethod) {
                    // TODO: Add some automatic extraction of parameters based on knowledge about methods.
                    case CoreMethod.Configuration:
                    ...
                    case CoreMethod.AddEntity: /// Number of parameters will vary (because they are derived from all <see cref="AgoRapideAttribute.IsObligatory"/> for the actual entity type)
                        if (e.method.Parameters.Count > 5) throw new InvalidCountException("Maximum of 5 " + method.EntityType.ToStringShort() + " -" + nameof(AgoRapideAttribute.IsObligatory) + "- allowed for " + e.method.A.CoreMethod + ". Resolution: Limit is arbitrarily set, may easily be increased in C# code.\r\nDetails: " + method.ToString());
                    ...
                    // TODO: Add some automatic extraction of parameters based on knowledge about methods.
                    case CoreMethod.EntityIndex: return HandleCoreMethodEntityIndex(e.method, e.parameters.GetValue(0, () => method.ToString()));
                    // TODO: Add some automatic extraction of parameters based on knowledge about methods.
                    ...
                    case CoreMethod.UpdateProperty: return HandleCoreMethodUpdateProperty(e.method, e.parameters.GetValue(0, () => method.ToString()), e.parameters.GetValue(1, () => method.ToString()), e.parameters.GetValue(2, () => method.ToString()));
                    // TODO: Add some automatic extraction of parameters based on knowledge about methods.
                    case CoreMethod.PropertyOperation: return HandleCoreMethodPropertyOperation(e.method, e.parameters.GetValue(0, () => method.ToString()), e.parameters.GetValue(1, () => method.ToString()));
                    // TODO: Add some automatic extraction of parameters based on knowledge about methods.
                    case CoreMethod.History: return HandleCoreMethodHistory(e.method, e.parameters.GetValue(0, () => method.ToString()));
                    default: throw new NotImplementedException(nameof(exactMatch) + " with " + nameof(e.method.A.CoreMethod) + " " + e.method.A.CoreMethod);
                }
                ...
                var docUrl = Util.Configuration.A.RootUrl + (request.ResponseFormat == ResponseFormat.HTML ? Util.Configuration.A.HTMLPostfixIndicatorWithoutLeadingSlash : "");
        ...
        public object HandleCoreMethodConfiguration(APIMethod method) {
            Log("");
            method.A.AssertCoreMethod(CoreMethod.Configuration);
        ...
        public object HandleCoreMethodAddEntity(APIMethod method, string p1, string p2, string p3, string p4, string p5) {
            ...
            method.A.AssertCoreMethod(CoreMethod.AddEntity);
        ...
        public object HandleCoreMethodEntityIndex(APIMethod method, string id) {
            ...
            method.A.AssertCoreMethod(CoreMethod.EntityIndex);
        ...
        public object HandleCoreMethodUpdateProperty(APIMethod method, string id, string key, string value) {
            ...
            method.A.AssertCoreMethod(CoreMethod.UpdateProperty);
            ...
            if (propertyKeyNonStrict.Key.A.IsUniqueInDatabase) {            
                ...
                if (!DB.TryAssertUniqueness(propertyKeyNonStrict.PropertyKey, objValue, out var existing, out var strErrorResponse)) return request.GetErrorResponse(ResultCode.data_error, strErrorResponse);
        ...
        public object HandleCoreMethodPropertyOperation(APIMethod method, string id, string operation) {
            ...
            method.A.AssertCoreMethod(CoreMethod.PropertyOperation);
        ...
        public object HandleCoreMethodHistory(APIMethod method, string id) {
            Log(nameof(id) + ": " + id);
            method.A.AssertCoreMethod(CoreMethod.History);
        ...
        public object HandleCoreMethodExceptionDetails(APIMethod method) {
            Log("");
            method.A.AssertCoreMethod(CoreMethod.ExceptionDetails);
            var request = new Request(Request, method, CurrentUser(method), exceptionHasOccurred: false);

            var logDirectory = System.IO.Path.GetDirectoryName(Util.Configuration.A.LogPath);
        ...
        protected bool TryGetRequest(
            ...
            { // Check if request is attempted HTML-format request misunderstood as JSON request with last parameter like /HTML
              ...
              // Call AgoRapideGenericMethod if that is the case. 
                ...
                if (lastParameter != null && lastParameter.ToLower().Equals(Util.Configuration.A.HTMLPostfixIndicatorWithoutLeadingSlashToLower)) {
