...
namespace AgoRapide.API {
    public abstract class BaseController<TProperty> : ApiController where TProperty : struct, IFormattable, IConvertible, IComparable { // What we really would want is "where T : Enum"

        protected abstract IDatabase<TProperty> DB { get; }
        ...
        ///    (note that even if they do not have <see cref="APIMethod{TProperty}.RequiresAuthorization"/> the client
        ...
        /// and will again be supposed to have <see cref="APIMethod{TProperty}.RequiresAuthorization"/> 
        ...
        /// Normally set. May be null if <paramref name="method"/> does not have <see cref="APIMethod{TProperty}.RequiresAuthorization"/>. 
        /// See comment for <paramref name="method"/>. 
        /// </param>
        /// <returns></returns>
        public object AgoRapideGenericMethod(APIMethod<TProperty> method, BaseEntityT<TProperty> currentUser) {
            ...
            var request = new Request<TProperty>(Request, method, currentUser, exceptionHasOccurred: false);
            var tip = request.ResponseFormat == ResponseFormat.HTML ? "\r\nTip: Add " + Util.Configuration.HTMLPostfixIndicator + " at the end of your request URL if you want this response in HTML-format instead of JSON" : "";
            ...
            if (!string.IsNullOrEmpty(Util.Configuration.ApiPrefix) && Util.Configuration.ApiPrefix.Length > 1) { // In principle length is guaranteed to be more than one when not empty
                ...
                if (!url.ToLower().Contains(prefix)) {
                    ...
                    request.Result.AddProperty(M(CoreProperty.Message), "Did you remember '" + Util.Configuration.ApiPrefix + "' in your URL?" + tip);
                    request.Result.AddProperty(M(CoreProperty.SuggestedUrl), suggestedUrl);
                    return request.GetResponse();
                }
            }

            Request<TProperty>.GetMethodsMatchingRequest(Request, request.ResponseFormat, out var exactMatch, out var candidateMatches, out var maybeIntended);

            if (exactMatch != null) {
                if (exactMatch.Item1.Origin != APIMethodOrigin.Autogenerated) throw new Request<TProperty>.MethodMatchingException("Found " + nameof(exactMatch) + " for " + exactMatch.Item1.Name + " with " + nameof(exactMatch.Item1.Origin) + " " + exactMatch.Item1.Origin + ". These methods are supposed to be handled by the 'ordinary' API .NET routing mechanism.");
                switch (exactMatch.Item1.A.A.CoreMethod) {
                    // TODO: Add some automatic extraction of parameters based on knowledge about methods.
                    case CoreMethod.AddEntity: /// Number of parameters will vary (because they are derived from all <see cref="AgoRapideAttribute.IsObligatory"/> for the actual entity type)
                        if (exactMatch.Item1.Parameters.Count > 5) throw new InvalidCountException("Maximum of 5 " + method.EntityType.ToStringShort() + " -" + nameof(AgoRapideAttribute.IsObligatory) + "- allowed for " + exactMatch.Item1.A.A.CoreMethod + ". Resolution: Limit is arbitrarily set, may easily be increased in C# code.\r\nDetails: " + method.ToString());
                        return HandleCoreMethodAddEntity(exactMatch.Item1,
                            exactMatch.Item1.Parameters.Count <= 0 ? null : exactMatch.Item2.GetValue(0, () => method.ToString()),
                            exactMatch.Item1.Parameters.Count <= 1 ? null : exactMatch.Item2.GetValue(1, () => method.ToString()),
                            exactMatch.Item1.Parameters.Count <= 2 ? null : exactMatch.Item2.GetValue(2, () => method.ToString()),
                            exactMatch.Item1.Parameters.Count <= 3 ? null : exactMatch.Item2.GetValue(3, () => method.ToString()),
                            exactMatch.Item1.Parameters.Count <= 4 ? null : exactMatch.Item2.GetValue(4, () => method.ToString())
                        // TODO: INCREASE LIMIT OF NUMBER OF PARAMETERS HERE!
                        );
                    // TODO: Add some automatic extraction of parameters based on knowledge about methods.
                    case CoreMethod.EntityIndex: return HandleCoreMethodEntityIndex(exactMatch.Item1, exactMatch.Item2.GetValue(0, () => method.ToString()));
                    // TODO: Add some automatic extraction of parameters based on knowledge about methods.
                    ...
                    case CoreMethod.UpdateProperty: return HandleCoreMethodUpdateProperty(exactMatch.Item1, exactMatch.Item2.GetValue(0, () => method.ToString()), exactMatch.Item2.GetValue(1, () => method.ToString()), exactMatch.Item2.GetValue(2, () => method.ToString()));
                    // TODO: Add some automatic extraction of parameters based on knowledge about methods.
                    case CoreMethod.PropertyOperation: return HandleCoreMethodPropertyOperation(exactMatch.Item1, exactMatch.Item2.GetValue(0, () => method.ToString()), exactMatch.Item2.GetValue(1, () => method.ToString()));
                    // TODO: Add some automatic extraction of parameters based on knowledge about methods.
                    case CoreMethod.History: return HandleCoreMethodHistory(exactMatch.Item1, exactMatch.Item2.GetValue(0, () => method.ToString()));
                    default: throw new NotImplementedException(nameof(exactMatch) + " with " + nameof(exactMatch.Item1.A.A.CoreMethod) + " " + exactMatch.Item1.A.A.CoreMethod);
                }
            } else if (candidateMatches != null) {
                switch (candidateMatches.Item1.Count) {
                    case 0: throw new InvalidCountException(nameof(candidateMatches) + ".Count: " + candidateMatches.Item1.Count);
                    case 1: {
                            var retval = new APIMethodCandidate<TProperty>(request, candidateMatches.Item1[0], candidateMatches.Item2);
                            ...
                            request.Result.AddProperty(M(CoreProperty.Message),
                                ...
                                tip);
                            request.Result.AddProperty(M(CoreProperty.SuggestedUrl), retval.PV<string>(M(CoreProperty.SuggestedUrl)));
                            return request.GetResponse();
                        }
                    default: {
                            // TODO: Order list by frequency of use and pick the most used methods
                            var retval = candidateMatches.Item1.Select(m => new APIMethodCandidate<TProperty>(request, m, candidateMatches.Item2)).Take(10).ToList();
                            request.Result.ResultCode = ResultCode.missing_parameter_error;
                            request.Result.MultipleEntitiesResult = retval.Select(r => (BaseEntityT<TProperty>)r).ToList();
                            request.Result.AddProperty(M(CoreProperty.Message),
                ...
                request.Result.AddProperty(M(CoreProperty.Message),
                    ...
                    "-----------\r\n" +
                    "Did you try to call one of the following methods:\r\n" +
                    string.Join("\r\n", maybeIntended.Item1.Select(c => c.ToString())) + "\r\n?" +
                    "-----------\r\n" +
                    "See enclosed " + nameof(CoreProperty.SuggestedUrl) + " for documentation" + tip);
                request.Result.MultipleEntitiesResult = maybeIntended.Item1.Select(m => (BaseEntityT<TProperty>)m).ToList();

                /// Generate <see cref="CoreProperty.SuggestedUrl"/>:
                ...
                request.Result.AddProperty(M(CoreProperty.SuggestedUrl), docUrl);
                request.Result.AddProperty(M(CoreProperty.APIDocumentationUrl), docUrl);
                return request.GetResponse();
            } else {
        ...
        public object HandleCoreMethodAddEntity(APIMethod<TProperty> method, string p1, string p2, string p3, string p4, string p5) {
            // TODO: INCREASE LIMIT OF NUMBER OF PARAMETERS HERE!
        ...
        /// <summary>
        /// TODO: For <see cref="APIMethod{TProperty}"/> THERE IS NO NEED OF READING FROM DATABASE!
        /// TODO: All these methods exists in memory anyway (see <see cref="APIMethod{TProperty}.AllMethods"/>)
        ...
        public object HandleCoreMethodUpdateProperty(APIMethod<TProperty> method, string id, string key, string value) {
            Log(nameof(id) + ": " + id + ", " + nameof(key) + ": " + key + ", " + nameof(value) + ": " + value);
            method.A.A.AssertCoreMethod(CoreMethod.UpdateProperty);
            ValidRequest<TProperty> request; object errorResponse; if (!TryGetRequest(id, key, value, method, out request, out errorResponse)) return errorResponse;
            var queryId = request.Parameters.PVM<QueryId<TProperty>>();
            var tPropertyKey = request.Parameters.PV<TProperty>(M(CoreProperty.Key));
            var strValue = request.Parameters.PV<string>(M(CoreProperty.Value));
            ...
            if (integerQueryId != null) request.Result.AddProperty(M(CoreProperty.SuggestedUrl), request.CreateAPIUrl(method.EntityType, integerQueryId.Id));
            request.Result.AddProperty(M(CoreProperty.Message), nameof(entities) + ".Count: " + entities.Count);
            return request.GetResponse();
        }
        ...
        public object HandleCoreMethodPropertyOperation(APIMethod<TProperty> method, string id, string operation) {
            ...
            var integerQueryId = queryId as IntegerQueryId<TProperty>;
            if (integerQueryId != null) request.Result.AddProperty(M(CoreProperty.SuggestedUrl), request.CreateAPIUrl(method.EntityType, integerQueryId.Id));
            request.Result.AddProperty(M(CoreProperty.Message), nameof(properties) + ".Count: " + properties.Count);
            return request.GetResponse();
        }
        ...
        /// indexed <see cref="APIMethod{TProperty}"/>'s <see cref="APIMethod{TProperty}.Controller"/> and 
        /// <see cref="APIMethod{TProperty}.ControllerMethod"/>
        /// </summary>
        /// <param name="caller"></param>
        /// <returns></returns>
        protected APIMethod<TProperty> GetMethod([System.Runtime.CompilerServices.CallerMemberName] string caller = "") => APIMethod<TProperty>.GetByControllerAndMethodName(GetType(), caller);

        protected bool TryGetRequest(out ValidRequest<TProperty> request, out object errorResponse, [System.Runtime.CompilerServices.CallerMemberName] string caller = "") => TryGetRequest(null, null, null, null, null, null, null, null, null, null, out request, out errorResponse, caller);
        protected bool TryGetRequest(APIMethod<TProperty> method, out ValidRequest<TProperty> request, out object errorResponse, [System.Runtime.CompilerServices.CallerMemberName] string caller = "") => TryGetRequest(null, null, null, null, null, null, null, null, null, method, out request, out errorResponse, caller);
        protected bool TryGetRequest(string p1, out ValidRequest<TProperty> request, out object errorResponse, [System.Runtime.CompilerServices.CallerMemberName] string caller = "") => TryGetRequest(p1, null, null, null, null, null, null, null, null, null, out request, out errorResponse, caller);
        protected bool TryGetRequest(string p1, APIMethod<TProperty> method, out ValidRequest<TProperty> request, out object errorResponse, [System.Runtime.CompilerServices.CallerMemberName] string caller = "") => TryGetRequest(p1, null, null, null, null, null, null, null, null, method, out request, out errorResponse, caller);
        protected bool TryGetRequest(string p1, string p2, out ValidRequest<TProperty> request, out object errorResponse, [System.Runtime.CompilerServices.CallerMemberName] string caller = "") => TryGetRequest(p1, p2, null, null, null, null, null, null, null, null, out request, out errorResponse, caller);
        protected bool TryGetRequest(string p1, string p2, APIMethod<TProperty> method, out ValidRequest<TProperty> request, out object errorResponse, [System.Runtime.CompilerServices.CallerMemberName] string caller = "") => TryGetRequest(p1, p2, null, null, null, null, null, null, null, method, out request, out errorResponse, caller);
        protected bool TryGetRequest(string p1, string p2, string p3, out ValidRequest<TProperty> request, out object errorResponse, [System.Runtime.CompilerServices.CallerMemberName] string caller = "") => TryGetRequest(p1, p2, p3, null, null, null, null, null, null, null, out request, out errorResponse, caller);
        protected bool TryGetRequest(string p1, string p2, string p3, APIMethod<TProperty> method, out ValidRequest<TProperty> request, out object errorResponse, [System.Runtime.CompilerServices.CallerMemberName] string caller = "") => TryGetRequest(p1, p2, p3, null, null, null, null, null, null, method, out request, out errorResponse, caller);
        protected bool TryGetRequest(string p1, string p2, string p3, string p4, out ValidRequest<TProperty> request, out object errorResponse, [System.Runtime.CompilerServices.CallerMemberName] string caller = "") => TryGetRequest(p1, p2, p3, p4, null, null, null, null, null, null, out request, out errorResponse, caller);
        protected bool TryGetRequest(string p1, string p2, string p3, string p4, APIMethod<TProperty> method, out ValidRequest<TProperty> request, out object errorResponse, [System.Runtime.CompilerServices.CallerMemberName] string caller = "") => TryGetRequest(p1, p2, p3, p4, null, null, null, null, null, method, out request, out errorResponse, caller);
        protected bool TryGetRequest(string p1, string p2, string p3, string p4, string p5, out ValidRequest<TProperty> request, out object errorResponse, [System.Runtime.CompilerServices.CallerMemberName] string caller = "") => TryGetRequest(p1, p2, p3, p4, p5, null, null, null, null, null, out request, out errorResponse, caller);
        protected bool TryGetRequest(string p1, string p2, string p3, string p4, string p5, APIMethod<TProperty> method, out ValidRequest<TProperty> request, out object errorResponse, [System.Runtime.CompilerServices.CallerMemberName] string caller = "") => TryGetRequest(p1, p2, p3, p4, p5, null, null, null, null, method, out request, out errorResponse, caller);
        /// <summary>
        ...
        protected bool TryGetRequest(
            ...
            APIMethod<TProperty> method,
            out ValidRequest<TProperty> request, out object errorResponse, [System.Runtime.CompilerServices.CallerMemberName] string caller = "") {
            if (method == null) method = GetMethod(caller);
            Log(caller + ", " + method.Name); // +
            ...
            //);
            BaseEntityT<TProperty> currentUser = null;
            ...
            { // Check if request is attempted HTML-format request misunderstood as JSON request with last parameter like /HTML
              ...
              // Call AgoRapideGenericMethod if that is the case. 
                ...
                if (lastParameter != null && lastParameter.ToLower().Equals(Util.Configuration.HTMLPostfixIndicatorWithoutLeadingSlashToLower)) {
                    // This check does not hold. We could check URL direct for /HTML/HTML though...
                    //var tempRequest = new Request<TProperty>(Request, method, currentUser, exceptionHasOccurred: false);
            ...
            var parameters = new Dictionary<TProperty, Property<TProperty>>();
            ...
            var checker = new Func<int, string, bool>((index, parameter) => {
                ...
                var p = method.Parameters[index - 1].Item1;
                var ea = method.Parameters[index - 1].Item2;
                if (parameter == null) parameter = ea.A.DefaultValue;
                if (parameter == null) { // Missing parameter and no default available
                    errorResponseTemp = new Request<TProperty>(Request, method, currentUser, exceptionHasOccurred: false).GetErrorResponse(
            ...
            errorResponse = null;
            request = new ValidRequest<TProperty>(Request, method, currentUser, new Parameters<TProperty>(parameters));
        ...
        /// TODO: OR RATHER NOT, we need to access it separately for <see cref="BaseController{TProperty}.AgoRapideGenericMethod"/>
        /// </summary>
        /// <param name="method"></param>
        /// <returns></returns>
        protected BaseEntityT<TProperty> CurrentUser(APIMethod<TProperty> method) => TryGetCurrentUser(method, out var currentUser, out _) ? currentUser : throw new UnauthorizedAccessException("Method: " + method.ToString());

        /// <summary>
        /// TODO: Put functionality for CurrentUser into TryGetRequest instead
        /// 
        /// Note that when User.Identity.Name refers to a person with <see cref="CoreProperty.EntityToRepresent"/> then that entity will be returned 
        /// 
        /// Note how uses Request.Properties["AgoRapideCurrentUser"] if that one exists.
        /// 
        /// Only returns false when <paramref name="currentUser"/>'s <see cref="CoreProperty.AccessLevelGiven"/> is below the level required. 
        /// In such a case the resulting <see cref="ResultCode.access_error"/> <paramref name="accessDeniedResponse"/> will have a 
        /// <see cref="CoreProperty.message"/> explaining problem in detail.
        /// 
        /// Throws exception in all other fail cases like invalid long for User.Identity.Name or entity not found in database
        /// </summary>
        /// <param name="currentUser"></param>
        /// <returns></returns>
        protected bool TryGetCurrentUser(APIMethod<TProperty> method, out BaseEntityT<TProperty> currentUser, out object accessDeniedResponse) {
            ...
            if (Request.Properties.TryGetValue("AgoRapideCurrentUser", out var objCurrentUser)) {
                ...
                retval = objCurrentUser as BaseEntityT<TProperty>;
                if (retval == null) throw new InvalidObjectTypeException(objCurrentUser, typeof(BaseEntityT<TProperty>));
                if (retval.Id != entityId) throw new Exception(nameof(retval) + "." + nameof(retval.Id) + " (" + retval.Id + ") != " + nameof(entityId) + " (" + entityId + ")");
            } else {
                retval = DB.GetEntityById<BaseEntityT<TProperty>>(entityId);
                DB.SwitchIfHasEntityToRepresent(ref retval);
            }
            if (retval.AccessLevelGiven < method.PV<AccessLevel>(M(CoreProperty.AccessLevelUse))) {
                accessDeniedResponse = null;
                var request = new Request<TProperty>(Request, method, retval, exceptionHasOccurred: false);
                accessDeniedResponse = request.GetAccessDeniedResponse(nameof(currentUser) + "." + nameof(retval.AccessLevelGiven) + ") (" + retval.AccessLevelGiven + ") is less than method." + nameof(CoreProperty.AccessLevelUse) + " (" + method.PV<AccessLevel>(M(CoreProperty.AccessLevelUse)) + ")");
                currentUser = null;
                return false;
            }
            accessDeniedResponse = null;
        ...
        //protected abstract string GetHTMLHeading(Request<TProperty> request, string title, ResultCode status);
        //protected abstract string GetHTMLFooter(Request<TProperty> request);
        ...
        protected object HandleExceptionAndGenerateResponse(Exception ex, [System.Runtime.CompilerServices.CallerMemberName] string caller = "") {
            ...
            // Since Method<TProperty>.GetByControllerAndMethodName may be just the call that failed we have to be very careful here.
            if (!APIMethod<TProperty>.TryGetByControllerAndMethodName(GetType(), caller, out var method)) {
                return JSONView<TProperty>.GenerateEmergencyResult(ResultCode.exception_error,
                    "Method " + GetType() + "." + caller + " not found.\r\n" +
                    "Very unexpected.\r\n" +
                    "Unable to call " + nameof(Request<TProperty>) + "." + nameof(Request<TProperty>.GetErrorResponse) + " in order to present the following exception information:\r\n" +
                    msg);
            } else { // Preferred variant, calls JSONView<TProperty>.GenerateResult via Request and Result classes.
                return new Request<TProperty>(Request, method, currentUser: null, exceptionHasOccurred: true).GetErrorResponse(ResultCode.exception_error, msg);
            }
        }
        ...
        /// <param name="caller"></param>
        /// <returns></returns>
        protected long GetId([System.Runtime.CompilerServices.CallerMemberName] string caller = "") => ApplicationPart<TProperty>.GetOrAdd<ClassAndMethod<TProperty>>(GetType(), caller, DB).Id;
