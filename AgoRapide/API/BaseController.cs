        ...
        public object AgoRapideGenericMethod(APIMethod method, BaseEntity currentUser) {
            ...
            if (!string.IsNullOrEmpty(Util.Configuration.ApiPrefix) && Util.Configuration.ApiPrefix.Length > 1) { // In principle length is guaranteed to be more than one when not empty
                ...
                if (!url.ToLower().Contains(prefix)) {
                    ...
                    request.Result.AddProperty(CoreP.Message.A(), "Did you remember '" + Util.Configuration.ApiPrefix + "' in your URL?" + tip);
            ...
            if (exactMatch != null) {
                ...
                switch (e.method.A.A.CoreMethod) {
                    ...
                    case CoreMethod.AddEntity: /// Number of parameters will vary (because they are derived from all <see cref="AgoRapideAttribute.IsObligatory"/> for the actual entity type)
                        if (e.method.Parameters.Count > 5) throw new InvalidCountException("Maximum of 5 " + method.EntityType.ToStringShort() + " -" + nameof(AgoRapideAttribute.IsObligatory) + "- allowed for " + e.method.A.A.CoreMethod + ". Resolution: Limit is arbitrarily set, may easily be increased in C# code.\r\nDetails: " + method.ToString());
                    ...
                    default: throw new NotImplementedException(nameof(exactMatch) + " with " + nameof(e.method.A.A.CoreMethod) + " " + e.method.A.A.CoreMethod);
                }
