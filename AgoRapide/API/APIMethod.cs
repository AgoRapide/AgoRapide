...
namespace AgoRapide.API {
    ...
    public class APIMethod : ApplicationPart {
        ...
        public APIMethod(bool requiresAuthorization, Type entityType, APIMethodAttribute methodAttribute, List<HTTPMethod> httpMethods, List<RouteSegmentClass> routeSegments, List<PropertyKey> parameters, string routeTemplate) : base(methodAttribute) {
            AddPropertyM(APIMethodOrigin.Autogenerated);
            AddProperty(CoreP.RequiresAuthorization.A(), requiresAuthorization);
        ...
        /// <param name="httpMethods"></param>
        public APIMethod(bool requiresAuthorization, Type controller, System.Reflection.MethodInfo controllerMethod, APIMethodAttribute methodAttribute, List<HTTPMethod> httpMethods) : base(methodAttribute) {
            AddPropertyM(APIMethodOrigin.SemiAutogenerated);
            AddProperty(CoreP.RequiresAuthorization.A(), requiresAuthorization);
            ...
            controllers.ForEach(controller => {
                controller.GetMethods().ForEach(methodInfo => {
                    ...
                    var method = new APIMethod(
                        requiresAuthorization: attributeClassesSignifyingRequiresAuthorization.Any(
                            t => null != Attribute.GetCustomAttribute(methodInfo, t)),
        ...
        public static void CreateAutogeneratedMethods(List<Type> types, IDatabase db) {
            ...
            var connector = new Action<APIMethod>(method => {
                /// TODO: Find mechanism for setting invalid for methods no longer relevant (that is, old methods resisting in database
                /// TODO: no longer relevant since ConnectMethodWithDatabase is not called for them)
            ...
            var entityIndexCreator = new Action<Type>(t => { // TODO: There appears to be too much duplicated code in the creators methods here
                ...
                var method = new APIMethod(
                    ...
                    requiresAuthorization: !typeof(ApplicationPart).IsAssignableFrom(t), /// Note how <see cref="ApplicationPart"/> like <see cref="ClassMember"/>, <see cref="EnumValue"/> and <see cref="APIMethod"/> can be read-accessed (queried) without authorization (that is, the API documentation is common available)
                    entityType: t,
                    methodAttribute: new APIMethodAttribute {
        ...
        [ClassMember(Description = 
                "1) Filters (ignores) if -" + nameof(APIMethodAttribute.Environment) + "- " +
                "   does not match -" + nameof(ConfigurationAttribute.Environment) + "- " +
                "   (but adds to -" + nameof(IgnoredMethods) + "- first).\r\n" +
                "2) Calls -" + nameof(APIMethod.ConnectWithDatabase) + "-.\r\n" +
                "3) Adds to -" + nameof(_allMethods) + "-.")]
        private static void FilterConnectWithDatabaseAndAddMethod(APIMethod method, IDatabase db) {
            ...
            method.MA.SetIdentifierAndName(new Func<(string Identifier, string Name)>(() => {
                switch (method.MA.CoreMethod) {
                    case CoreAPIMethod.RootIndex:
                        return (
                            method.MA.GetType().ToStringShort().Replace("Attribute", "") + "_" + method.MA.CoreMethod.ToString(),
                            "(" + method.MA.CoreMethod + ")" /// <see cref="APIMethod.RouteTemplates"/> can not be used as it is empty
                        );
                    case CoreAPIMethod.GenericMethod:
                        return (
                            method.MA.GetType().ToStringShort().Replace("Attribute", "") + "_" + method.MA.CoreMethod.ToString(),
                            method.RouteTemplates[0]
                        );
                    default:
                        return (
                           method.MA.GetType().ToStringShort().Replace("Attribute", "") + "_" + method.RouteTemplates[0].Replace("/", "_").Replace("{", "_").Replace("}", "_"),
                           method.RouteTemplates[0]
                       );
                }
            })());
            ...
            updater(CoreP.APIMethodOrigin, method.PVM<APIMethodOrigin>());
            updater(CoreP.RouteTemplate, method.RouteTemplates[0]);
            switch (method.PVM<APIMethodOrigin>()) {
                case APIMethodOrigin.Autogenerated: updater(CoreP.Implementator, nameof(BaseController) + ".HandleCoreMethod" + method.MA.CoreMethod); break;
                default: updater(CoreP.Implementator, method.Controller.ToStringShort() + "." + method.ControllerMethod.Name); break;
            }
