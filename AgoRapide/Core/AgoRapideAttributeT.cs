...
namespace AgoRapide.Core {
    /// <summary>
    /// TODO: Make this inherit <see cref="BaseEntityT{TProperty}"/> and store Properties to database. In this manner we
    ...
    /// TODO: ACTUALLY, make this inherit <see cref="ApplicationPart{TProperty}"/> (with its own id against database)
    /// TODO: and with its own <see cref="CoreMethod"/> called AgoRapideAttribute.
    /// 
    /// TODO: (AFTER IMPLEMENTING ABOVE) MOVE THIS TO ENTITY-MAP SINCE INHERITS <see cref="BaseEntityT{TProperty}"/> 
    /// 
    ...
    public class AgoRapideAttributeT<TProperty> where TProperty : struct, IFormattable, IConvertible, IComparable { // What we really would want is "where T : Enum"
        ...
        public string PExplained => _pExplained ?? (_pExplained = CoreProperty == CoreProperty.None ?
            (typeof(TProperty).ToStringVeryShort() + "." + P.ToString()) :
            (nameof(CoreProperty) + "." + CoreProperty + " (mapped to " + typeof(TProperty).ToStringVeryShort() + "." + P.ToString() + ")"));
        ...
        /// Used by <see cref="Extensions.GetObligatoryChildProperties{TProperty}(Type)"/>. 
        /// </summary>
        ...
        /// In other cases use <see cref="AgoRapideAttributeT{TProperty}.AgoRapideAttributeT(AgoRapideAttribute)"/>
        /// </summary>
        /// <param name="agoRapideAttribute"></param>
        /// <param name="p">
        ...
        /// </param>
        public AgoRapideAttributeT(AgoRapideAttribute agoRapideAttribute, TProperty? p) {
            ...
            var isAttributeForCorePropertyItself = typeof(CoreProperty).Equals(typeof(TProperty));
            var isSilentlyMappedCoreProperty = (p != null);

            if (isSilentlyMappedCoreProperty) {
                if (!(A.Property is CoreProperty)) throw new InvalidObjectTypeException(A.Property, typeof(CoreProperty), nameof(A) + "." + nameof(A.Property) + " is not of type " + typeof(TProperty) + " but " + A.Property.GetType() + ". Check initialisation of " + nameof(AgoRapideAttribute) + "\r\nDetails: " + agoRapideAttribute.ToString());
                P = (TProperty)(p ?? throw new NullReferenceException(nameof(p) + ", should never happen, was just checked !null"));
            } else {
                if (!(A.Property is TProperty)) throw new InvalidObjectTypeException(A.Property, typeof(TProperty), nameof(A) + "." + nameof(A.Property) + " is not of type " + typeof(TProperty) + " but " + A.Property.GetType() + ". Check initialisation of " + nameof(AgoRapideAttribute) + "\r\nDetails: " + agoRapideAttribute.ToString());
                P = (TProperty)A.Property;
            }
            ...
            /// -----------------------------------------
            if (A.Group != null) {
                InvalidTypeException.AssertAssignable(A.Group, typeof(IGroupDescriber), () => "Type given as " + typeof(AgoRapideAttribute).ToString() + "." + nameof(AgoRapideAttribute.Group) + " to " + typeof(TProperty).ToString() + "." + A.Property + " must implement " + typeof(Core.IGroupDescriber));
                try {
                    ((IGroupDescriber)Activator.CreateInstance(A.Group)).EnrichAttribute(this);
                } catch (Exception ex) {
                    throw new AgoRapideAttributeException(
                        "Unable to initialize instance of " + A.Group + " given as " + typeof(AgoRapideAttribute).ToString() + "." + nameof(AgoRapideAttribute.Group) + " to " + typeof(TProperty).ToString() + "." + A.Property + ".\r\n" +
            ...
            if (A.Type != null && typeof(ITypeDescriber).IsAssignableFrom(A.Type)) {
                ...
                try {
                    ...
                    throw new AgoRapideAttributeException(
                        ...
                        "given as " + typeof(AgoRapideAttribute).ToString() + "." + nameof(AgoRapideAttribute.Type) + " to " + typeof(TProperty).ToString() + "." + A.Property + "\r\n" +
                        "Resolution: Check that it exists and that it takes exactly one parameter of type " + typeof(AgoRapideAttributeT<TProperty>).ToStringShort() + ", in other words it should look like\r\n" +
                        "   public static void method " + methodName + "<TProperty>(AgoRapideAttributeT<TProperty>) where TProperty : struct, IFormattable, IConvertible, IComparable\r\n" +
                        "details:\r\n" + agoRapideAttribute.ToString(), ex);
                }
            }
            ...
            if (ValidatorAndParser != null) {
                ...
                if (A.Type == null) {
                    ...
                    if (A.Type.Equals(typeof(string))) {
                        ValidatorAndParser = value => !string.IsNullOrEmpty(value) ? new ParseResult<TProperty>(new Property<TProperty>(P, value), value) : new ParseResult<TProperty>("Illegal as string (" + (value == null ? "[NULL]" : "[EMPTY]") + ")");
                    } else if (A.Type.Equals(typeof(int))) {
                        throw new TypeIntNotSupportedByAgoRapideException(A.ToString());
                        // ValidatorAndParser = value => int.TryParse(value, out var intValue) ? new ParseResult<TProperty>(new Property<TProperty>(P, intValue), intValue) : new ParseResult<TProperty>("Illegal as int");
                    } else if (A.Type.Equals(typeof(long))) {
                        ValidatorAndParser = value => long.TryParse(value, out var lngValue) ? new ParseResult<TProperty>(new Property<TProperty>(P, lngValue), lngValue) : new ParseResult<TProperty>("Illegal as long");
                        ...
                        ValidatorAndParser = value => bool.TryParse(value, out var blnValue) ? new ParseResult<TProperty>(new Property<TProperty>(P, blnValue), blnValue) : new ParseResult<TProperty>("Illegal as boolean, use '" + true.ToString() + "' or '" + false.ToString() + "'");
                        ...
                        ValidatorAndParser = value => DateTime.TryParseExact(value, validFormats, Util.Configuration.Culture, System.Globalization.DateTimeStyles.None, out var dtmValue) ? new ParseResult<TProperty>(new Property<TProperty>(P, dtmValue), dtmValue) : new ParseResult<TProperty>(
                            "Invalid as " + A.Type + ".\r\n" +
                            "Must be in one of the following formats:\r\n" +
                            string.Join(", ", validFormats) + "\r\n");
                    } else if (A.Type.IsEnum) {
                        ValidatorAndParser = value => Util.EnumTryParse(A.Type, value, out var enumValue) ? new ParseResult<TProperty>(new Property<TProperty>(P, enumValue), enumValue) : new ParseResult<TProperty>(
                            "Invalid as " + A.Type + ".\r\n" +
                            "Must be one of\r\n" +
        ...
        /// 
        /// Typically used before calling <see cref="APIMethod{TProperty}.GetAPICommand"/>
        /// 
        ...
        public string ConvertObjectToString(object obj) {
            ...
            return obj.ToString();
            //throw new InvalidTypeException(A.Type, "Not implemented for other than string or integer. Details: " + ToString());
        }
        ...
        private static Type TranslateType(Type type) {
            ...
            if (type.Equals(typeof(QueryId<CoreProperty>))) return typeof(QueryId<TProperty>);
            if (type.Equals(typeof(IntegerQueryId<CoreProperty>))) return typeof(IntegerQueryId<TProperty>);
            if (type.Equals(typeof(PropertyValueQueryId<CoreProperty>))) return typeof(PropertyValueQueryId<TProperty>);
            if (type.Equals(typeof(Request<CoreProperty>))) return typeof(Request<TProperty>); // 

            /// Ordinary <see cref="BaseEntityT{TProperty}"/> classes:
            if (type.Equals(typeof(APIMethod<CoreProperty>))) return typeof(APIMethod<TProperty>);
            if (type.Equals(typeof(APIMethodCandidate<CoreProperty>))) return typeof(APIMethodCandidate<TProperty>);
            if (type.Equals(typeof(ApplicationPart<CoreProperty>))) return typeof(ApplicationPart<TProperty>);
            if (type.Equals(typeof(BaseEntityT<CoreProperty>))) return typeof(BaseEntityT<TProperty>);
            if (type.Equals(typeof(BaseEntityTWithLogAndCount<CoreProperty>))) return typeof(BaseEntityTWithLogAndCount<TProperty>);
            if (type.Equals(typeof(ClassAndMethod<CoreProperty>))) return typeof(ClassAndMethod<TProperty>);
            if (type.Equals(typeof(EnumClass<CoreProperty>))) return typeof(EnumClass<TProperty>);
            if (type.Equals(typeof(GeneralQueryResult<CoreProperty>))) return typeof(GeneralQueryResult<TProperty>);
            if (type.Equals(typeof(Parameters<CoreProperty>))) return typeof(Parameters<TProperty>);
            if (type.Equals(typeof(Property<CoreProperty>))) return typeof(Property<TProperty>);
            if (type.Equals(typeof(Result<CoreProperty>))) return typeof(Result<TProperty>);
        ...
        /// 
        /// Note that for some types (like long, double, bool, DateTime, string) <see cref="AgoRapideAttributeT{TProperty}(AgoRapideAttribute)"/> 
        ...
        /// <summary>
        /// Note that for some types (like long, double, bool, DateTime) <see cref="AgoRapideAttributeT{TProperty}(AgoRapideAttribute)"/> 
        ...
        /// </summary>
        public Func<string, ParseResult<TProperty>> ValidatorAndParser { get; set; }
        ...
        /// <returns></returns>
        public bool TryValidateAndParse(string value, out ParseResult<TProperty> result) {
            A.AssertProperty();
            /// TODO: This is not good enough. We should not accept not having a validator and parser...
            // if (ValidatorAndParser == null) { result = new ParseResult<TProperty>(new Property<TProperty>(P, value)); return true; }
            ...
            if (result.ErrorResponse != null) result = new ParseResult<TProperty>("Validation failed for " + PToString + " = '" + value + "'.\r\nDetails: " + result.ErrorResponse);
        ...
        /// <returns></returns>
        public bool TryCleanAndValidateAndParse(string value, out ParseResult<TProperty> result) {
            A.AssertProperty();
    ...
    /// </summary>
    public class ParseResult<TProperty> where TProperty : struct, IFormattable, IConvertible, IComparable { // What we really would want is "where T : Enum"

        public Property<TProperty> Result { get; private set; }

        /// <summary>
        /// Corresponds to <see cref="Result"/>.<see cref="Property{TProperty}.ADotTypeValue"/>
        /// TODO: Ideally we would like to do without this parameter but that would lead to <see cref="Property{TProperty}.ADotTypeValue"/> 
        /// calling itself 
        /// (see code line
        ...
        public ParseResult(Property<TProperty> result, object objResult) {
